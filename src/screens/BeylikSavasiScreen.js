...
window.BeylikSavasiScreen = function BeylikSavasiScreen({ profile, token, serverBeyliks }) {
  ...
  const [expandedSchemeId, setExpandedSchemeId] = React.useState(null);
    ...
    tab === 'aktif' && React.createElement('div', { style:{ display:'flex', flexDirection:'column', gap:'10px' }},
      ...
      wars.map(w => {
        ...
        return React.createElement('div', { key:w.id, style:{ background:ds.surface, border:`1px solid rgba(201,162,39,0.25)`, borderRadius:'12px', padding:'14px' }},
          ...
          window.BattlefieldScheme && React.createElement('button', {
            onClick: () => setExpandedSchemeId(expandedSchemeId === w.id ? null : w.id),
            style:{ width:'100%', marginBottom:'8px', padding:'7px', borderRadius:'8px', border:`1px solid ${ds.gold}`, background:'rgba(201,162,39,0.08)', color:ds.gold, cursor:'pointer', fontWeight:700, fontSize:'0.76rem', fontFamily:'inherit' }
          }, expandedSchemeId === w.id ? '▲ Savaş Şemasını Gizle' : '🗺️ Savaş Alanı Şemasını Gör'),
          window.BattlefieldScheme && expandedSchemeId === w.id && React.createElement('div', { style:{ marginBottom:'8px' }},
            React.createElement(window.BattlefieldScheme, { war: w })
          ),
