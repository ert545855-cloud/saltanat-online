...
function ArmyPage({ profile, setProfile, showNotif }) {
  ...
  const UNIT_BADGES = { infantry:'/assets/badges/piyade-badge.svg', cavalry:'/assets/badges/suvari-badge.svg' };
  const UnitIcon = ({unit, size}) => UNIT_BADGES[unit.id]
    ? <img src={UNIT_BADGES[unit.id]} width={size||28} height={size||28} style={{borderRadius:'50%',display:'block'}} alt={unit.name}/>
    : <span style={{fontSize:(size||28)*0.6+'px'}}>{unit.icon}</span>;
        ...
        <div style={{background:'rgba(237,231,218,0.02)',border:'1px solid rgba(237,231,218,0.08)',borderRadius:'12px',padding:'1rem'}}>
          ...
          {UNITS.map(u2=>(
            <div key={u2.id} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'0.4rem 0.5rem',borderRadius:'6px',marginBottom:'0.25rem',background:'rgba(237,231,218,0.02)'}}>
              <div style={{display:'flex',alignItems:'center',gap:'0.5rem'}}><UnitIcon unit={u2} size={24}/><span style={{fontWeight:600,fontSize:'0.85rem'}}>{u2.name}</span></div>
        ...
        {UNITS.map(unit=>(
          <div key={unit.id} style={{background:'rgba(237,231,218,0.02)',border:'1px solid rgba(237,231,218,0.08)',borderRadius:'12px',padding:'1rem',marginBottom:'0.75rem'}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'0.5rem'}}>
              <div style={{display:'flex',alignItems:'center',gap:'0.5rem'}}><UnitIcon unit={unit} size={34}/><div><div style={{fontWeight:700,fontSize:'0.9rem'}}>{unit.name}</div><div style={{fontSize:'0.7rem',color:'#999'}}>Güç: {unit.strength} · Bakım: 🪙{unit.upkeep.toLocaleString()}/gün · Adet: {myArmy[unit.id]||0}</div></div></div>
