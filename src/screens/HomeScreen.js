function HomePage({ profile, onNavigate }) {
  ...
  const dynamicActivity = [
    { text:`${profile?.username||'Sen'} oyuna giriş yaptı`, color:'#C89B3C', time:'Az önce' },
    { text:'Şehirde yeni bir bina inşa edildi', color:'#C89B3C', time:'5dk' },
    { text:'Çete savaşı başladı', color:'#C2453D', time:'12dk' },
    { text:`Lv.${playerLevel} — ${lvl.title}`, color:'#4C9A6B', time:'1sa' },
    { text:'Borsa: TECH +2.4%', color:'#C89B3C', time:'2sa' },
  ];
  ...
  const catColor = {Siyaset:'#C2453D',Ekonomi:'#C89B3C',Hukuk:'#C89B3C',Etkinlik:'#4C9A6B',Sistem:'#C89B3C'};
  ...
  return (
    <div style={{padding:'0 0.75rem 1rem',background:'#1A0E00',minHeight:'100%'}}>
      {/* ── Welcome card (stays dark) ── */}
      <div style={{borderRadius:'20px',marginBottom:'0.75rem',border:'1px solid rgba(200,155,60,0.15)',boxShadow:'0 8px 28px rgba(0,0,0,0.45), inset 0 1px 0 rgba(255,255,255,0.04)',marginTop:'0.75rem',overflow:'hidden'}}>
        {/* Banner */}
        {profile?.bannerUrl && (
          <div style={{height:'90px',backgroundImage:`url(${profile.bannerUrl})`,backgroundSize:'cover',backgroundPosition:'center',position:'relative'}}>
            <div style={{position:'absolute',inset:0,background:'linear-gradient(to bottom,rgba(0,0,0,0.05),rgba(45,24,0,0.85))'}}/>
            {/* avatar üst sağ */}
        ...
        <div style={{background:'#2D1800',padding:'1.2rem',paddingTop: profile?.bannerUrl ? '1.5rem' : '1.2rem'}}>
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'0.75rem'}}>
            <div>
              <div style={{fontSize:'0.72rem',color:'#A9A6A0',marginBottom:'0.2rem',fontWeight:600}}>{T('playerProfile')}</div>
              <div style={{display:'flex',alignItems:'center',gap:6}}>
                <div style={{fontFamily:"'Cinzel',serif",fontSize:'1.35rem',fontWeight:900,color:'#F5EBD7'}}>{profile?.username||'Oyuncu'}</div>
              ...
              <div style={{fontSize:'0.65rem',color:'#A9A6A0',marginTop:'0.1rem'}}>{lvl.title} • {lvl.pct}% {T('nextLevel')}</div>
            </div>
            <div style={{display:'flex',alignItems:'center',gap:'0.5rem'}}>
              ...
              <button onClick={()=>onNavigate('oyun_ayarlari')} title="Oyun Ayarları" style={{background:'rgba(200,155,60,0.1)',border:'1px solid rgba(200,155,60,0.25)',borderRadius:'10px',padding:'8px 10px',color:'#C89B3C',fontSize:'1.1rem',cursor:'pointer',lineHeight:1,display:'flex',flexDirection:'column',alignItems:'center',gap:'2px'}}>
                ⚙️
        ...
        <div style={{marginBottom:'0.6rem'}}>
          <div style={{display:'flex',justifyContent:'space-between',fontSize:'0.6rem',color:'#A9A6A0',marginBottom:'4px'}}>
            <span>{lvl.xp?.toLocaleString()} XP</span><span>→ {lvl.next?.xp?.toLocaleString()} XP</span>
          </div>
          <div style={{height:'4px',background:'rgba(245,235,215,0.07)',borderRadius:'2px',overflow:'hidden'}}>
            <div style={{height:'100%',width:`${lvl.pct}%`,background:'linear-gradient(90deg,#C89B3C,#E8C06A)',borderRadius:'2px',transition:'width 0.5s'}}/>
          </div>
        </div>
        <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'0.5rem'}}>
          {[
            ...
            <div key={label} style={{textAlign:'center',background:'rgba(245,235,215,0.05)',borderRadius:'10px',padding:'0.5rem 0.2rem'}}>
              <div style={{fontSize:'0.75rem',marginBottom:'0.1rem'}}>{icon}</div>
              <div style={{fontSize:'0.52rem',color:'#A9A6A0',textTransform:'uppercase',letterSpacing:'0.07em',fontWeight:700,marginBottom:'0.1rem'}}>{label}</div>
              <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:'1rem',fontWeight:900,color:'#F5EBD7'}}>{value}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div onClick={()=>onNavigate('sadakat')} style={{background:'linear-gradient(135deg,rgba(200,155,60,0.14),rgba(45,24,0,0.9))',border:'1px solid rgba(200,155,60,0.28)',borderRadius:'10px',padding:'0.75rem 1rem',marginBottom:'0.55rem',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'space-between',WebkitTapHighlightColor:'transparent'}}>
        <div style={{display:'flex',alignItems:'center',gap:'0.5rem'}}>
          ...
          <div>
            <div style={{fontSize:'0.6rem',color:'#A9A6A0',textTransform:'uppercase',letterSpacing:'0.08em',fontWeight:700}}>Sadakat Puanı</div>
        ...
        <div style={{textAlign:'right'}}>
          <div style={{fontSize:'0.6rem',color:'#A9A6A0',marginBottom:3}}>Beylik kurmak için 150.000</div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'0.55rem',marginBottom:'0.75rem'}}>
        {[
          {icon:'🔔',label:T('notifications'),value:unreadCount||0,sub:unreadCount>0?`${unreadCount} ${T('unread')}`:T('allClaimed'),color:'#C89B3C',page:'chat'},
          {icon:'🏆',label:T('achievements'),value:`${earnedAch}/${allAchievements}`,sub:`%${Math.round(earnedAch/(allAchievements||1)*100)} tamamlandı`,color:'#C89B3C',page:'achievements'},
          {icon:'📈',label:T('economy'),value:portfolioChange,sub:T('portfolio'),color:'#4C9A6B',positive:true,page:'economy'},
          {icon:'⚡',label:T('players'),value:onlineCnt||0,sub:T('online'),color:'#C89B3C',page:'players'},
        ].map((item)=>(
          <div key={item.label} onClick={()=>onNavigate(item.page)}
            style={{background:'#2D1800',border:'1px solid rgba(245,235,215,0.08)',borderRadius:'10px',padding:'0.9rem 0.85rem',cursor:'pointer',transition:'all 0.15s',WebkitTapHighlightColor:'transparent'}}>
            <div style={{display:'flex',alignItems:'center',gap:'0.35rem',marginBottom:'0.35rem'}}>
              <span style={{fontSize:'1rem'}}>{item.icon}</span>
              <span style={{fontSize:'0.65rem',fontWeight:700,color:'#A9A6A0',textTransform:'uppercase',letterSpacing:'0.05em'}}>{item.label}</span>
            </div>
            {(() => { const LV = window.LedgerValue; return LV
              ? React.createElement(LV, {value:item.value, prefix:false, color:item.positive?'#4C9A6B':'#F5EBD7', style:{alignItems:'flex-start',background:'none',border:'none',padding:'0',marginBottom:'0.2rem'}})
              : React.createElement('div',{style:{fontFamily:"'JetBrains Mono',monospace",fontSize:'1.3rem',fontWeight:900,color:item.positive?'#4C9A6B':'#F5EBD7',lineHeight:1,marginBottom:'0.2rem'}},item.value);
            })()}
            <div style={{fontSize:'0.63rem',color:'#A9A6A0'}}>{item.sub}</div>
          </div>
        ))}
      </div>
      ...
      {streakData !== null && (() => {
        ...
        return (
          <div style={{background:'#2D1800',borderRadius:'10px',padding:'1rem',marginBottom:'0.75rem',boxShadow:'0 4px 16px rgba(0,0,0,0.15)'}}>
            <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'0.65rem'}}>
              <div style={{display:'flex',alignItems:'center',gap:'0.5rem'}}>
                <span style={{fontSize:'1.4rem'}}>🔥</span>
                <div>
                  <div style={{fontFamily:"'Cinzel',serif",fontSize:'0.95rem',fontWeight:800,color:'#F5EBD7'}}>{streak} Günlük Seri</div>
              ...
              {!claimed ? (
                <button onClick={claimStreakAPI} disabled={streakClaiming}
                  style={{background:'linear-gradient(135deg,#C89B3C,#A07D1C)',border:'none',borderRadius:'10px',padding:'0.5rem 1rem',color:'#F5EBD7',fontFamily:"'Inter',sans-serif",fontWeight:800,fontSize:'0.78rem',cursor:'pointer'}}>
            ...
            <div style={{display:'flex',gap:'4px'}}>
              {[1,2,3,4,5,6,7].map(d=>{
                ...
                return (
                  <div key={d} style={{flex:1,height:'6px',borderRadius:'100px',background:active?'#C89B3C':isToday?'rgba(200,155,60,0.3)':'rgba(255,255,255,0.08)',transition:'all 0.3s'}} />
                );
              })}
            </div>
            {!claimed && <div style={{fontSize:'0.62rem',color:'#A9A6A0',marginTop:'0.4rem'}}>Yarın: +🪙{nextReward.toLocaleString('tr-TR')} • Zinciri kırma!</div>}
          </div>
        );
      })()}

      {/* ── Daily Tasks ── */}
      <div style={{background:'#2D1800',border:'1px solid rgba(245,235,215,0.08)',borderRadius:'10px',padding:'1rem',marginBottom:'0.75rem',boxShadow:'none'}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'0.8rem'}}>
          <div style={{fontFamily:"'Cinzel',serif",fontSize:'1rem',fontWeight:800,color:'#F5EBD7'}}>{T('dailyTasks')}</div>
          <span style={{fontSize:'0.65rem',color:'#A9A6A0',fontWeight:600}}>{todayKey}</span>
        </div>
        {taskList.map((task,i)=>{
          const prog = todayProgress[task.id] || 0;
            ...
            <div key={task.id} style={{marginBottom:i<taskList.length-1?'0.85rem':'0'}}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'0.25rem'}}>
                <div style={{display:'flex',alignItems:'center',gap:'0.4rem'}}>
                  ...
                  <span style={{fontSize:'0.82rem',fontWeight:600,color:claimed?'#A9A6A0':'#F5EBD7',textDecoration:claimed?'line-through':'none'}}>{task.name}</span>
                </div>
                {claimed ? (
                  <span style={{fontSize:'0.65rem',color:'#4C9A6B',fontWeight:800}}>✅ Alındı</span>
                ) : done ? (
                  <button onClick={(e)=>claimTask(task,e)} style={{fontSize:'0.65rem',fontWeight:800,color:'#F5EBD7',background:'#4C9A6B',border:'none',borderRadius:'8px',padding:'0.2rem 0.55rem',cursor:'pointer'}}>🎁 Al</button>
                ) : (
                  <button onClick={(e)=>{e.stopPropagation();onNavigate(task.page);}} style={{fontSize:'0.65rem',fontWeight:700,color:'#C89B3C',background:'none',border:'none',cursor:'pointer'}}>→ Git</button>
                )}
              </div>
              <div style={{display:'flex',alignItems:'center',gap:'0.5rem'}}>
                <div style={{flex:1,height:'5px',background:'rgba(245,235,215,0.1)',borderRadius:'100px',overflow:'hidden'}}>
                  <div style={{height:'100%',width:`${pct}%`,background:claimed?'#4C9A6B':'linear-gradient(90deg,#C89B3C,#C89B3C)',borderRadius:'100px',transition:'width 0.5s'}} />
                </div>
                <span style={{fontSize:'0.6rem',color:'#A9A6A0',fontWeight:600,flexShrink:0}}>{prog}/{task.target}</span>
              </div>
              <div style={{fontSize:'0.6rem',color:'#A9A6A0',marginTop:'0.15rem'}}>🎁 +🪙{task.reward.toLocaleString('tr-TR')} • +{task.xpReward} XP</div>
            </div>
          );
        })}
      </div>
      ...
      <div style={{background:'#2D1800',border:'1px solid rgba(245,235,215,0.08)',borderRadius:'10px',padding:'1rem',marginBottom:'0.75rem',boxShadow:'none'}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'0.7rem'}}>
          <div style={{fontFamily:"'Cinzel',serif",fontSize:'1rem',fontWeight:800,color:'#F5EBD7'}}>{T('announcements2')}</div>
          <button onClick={()=>onNavigate('duyurular')} style={{fontSize:'0.68rem',color:'#C89B3C',fontWeight:700,background:'none',border:'none',cursor:'pointer'}}>Tümü →</button>
        </div>
        {annList.slice(0,3).map(a=>(
          <button key={a.id||a.ts} onClick={()=>setAnnModal(a)}
            ...
            <div style={{flex:1,overflow:'hidden'}}>
              <div style={{fontSize:'0.82rem',fontWeight:700,color:'#F5EBD7',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{a.title}</div>
              <div style={{fontSize:'0.68rem',color:catColor[a.category]||'#A9A6A0',fontWeight:600}}>{a.category} • {timeAgo(a.ts)}</div>
            </div>
            <span style={{fontSize:'0.7rem',color:'#A9A6A0',flexShrink:0}}>›</span>
          </button>
        ))}
      </div>

      {/* ── Recent Activity ── */}
      <div style={{background:'#2D1800',border:'1px solid rgba(245,235,215,0.08)',borderRadius:'10px',padding:'1rem',boxShadow:'none',marginBottom:'0.75rem'}}>
        <div style={{fontFamily:"'Cinzel',serif",fontSize:'1rem',fontWeight:800,color:'#F5EBD7',marginBottom:'0.7rem'}}>Son Aktiviteler</div>
        {recentActivity.map((item,i,arr)=>(
          <div key={i} style={{display:'flex',alignItems:'center',gap:'0.6rem',padding:'0.55rem 0',borderBottom:i<arr.length-1?'1px solid rgba(0,0,0,0.05)':'none'}}>
            <div style={{width:'8px',height:'8px',borderRadius:'50%',background:item.color||'#C89B3C',flexShrink:0}} />
            <span style={{flex:1,fontSize:'0.82rem',color:'#A9A6A0',fontWeight:500}}>{item.text||item.desc||item.content||'Aktivite'}</span>
            <span style={{fontSize:'0.67rem',color:'#A9A6A0'}}>{item.time||timeAgo(item.ts)}</span>
          </div>
        ))}
      </div>
      ...
      <div style={{marginBottom:'0.75rem'}}>
        <div style={{fontFamily:"'Cinzel',serif",fontSize:'0.72rem',fontWeight:700,color:'#C89B3C',textTransform:'uppercase',letterSpacing:'0.1em',marginBottom:'0.45rem',display:'flex',alignItems:'center',gap:6}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'0.45rem'}}>
          {[
            ...
            {icon:'📋',label:'Günlük Görev',page:'gunluk_gorev',color:'#C89B3C',desc:'Ödül kazan'},
            {icon:'⚡',label:'Hızlı Merkez',page:'hizli_merkez',color:'#60A5FA',desc:'Transfer & stat'},
          ].map((a) => (
            <button key={a.page} onClick={() => onNavigate(a.page)}
              style={{background:'#2D1800',border:`1px solid ${a.color}22`,borderRadius:'14px',padding:'0.75rem 0.5rem',display:'flex',flexDirection:'column',alignItems:'center',gap:'0.2rem',cursor:'pointer',WebkitTapHighlightColor:'transparent',transition:'all 0.15s',position:'relative',overflow:'hidden'}}>
              ...
              <span style={{fontSize:'0.68rem',fontWeight:800,color:'#F5EBD7',textAlign:'center',lineHeight:1.2}}>{a.label}</span>
              <span style={{fontSize:'0.56rem',color:'#A9A6A0',textAlign:'center'}}>{a.desc}</span>
            </button>
          ))}
        </div>
      </div>
      ...
      <div style={{marginBottom:'0.75rem'}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'0.45rem'}}>
          {[
            ...
            {icon:'🤝',label:'Lonca Anlaşma',page:'lonca_anlasma',color:'#C89B3C',desc:'Ticaret yap'},
          ].map((a) => (
            <button key={a.page} onClick={() => onNavigate(a.page)}
              style={{background:'#2D1800',border:`1px solid ${a.color}22`,borderRadius:'14px',padding:'0.75rem 0.5rem',display:'flex',flexDirection:'column',alignItems:'center',gap:'0.2rem',cursor:'pointer',WebkitTapHighlightColor:'transparent',transition:'all 0.15s',position:'relative',overflow:'hidden'}}>
              <div style={{position:'absolute',top:0,left:0,right:0,height:'2px',background:a.color,opacity:0.6}} />
      ...
      <div style={{marginBottom:'0.75rem'}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'0.45rem'}}>
          {[
            {icon:'🕵️',label:'Casus Zinciri',page:'casus_chain',color:'#C2453D',desc:'3 aşamalı görev'},
      ...
      <div style={{marginBottom:'0.75rem'}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'0.45rem'}}>
          {[
            ...
            {icon:'🎖️',label:'Unvanlar',  page:'unvan_sistemi', color:'#C89B3C'},
            {icon:'🏅',label:'Rozetler',  page:'rozet_koleksiyon',color:'#C89B3C'},
            ...
            {icon:'🔔',label:'Bildirimler',page:'bildirim_gecmisi',color:'#C89B3C'},
          ].map((a,i)=>(
            <button key={i} onClick={()=>onNavigate(a.page)}
              style={{background:'#2D1800',border:`1px solid ${a.color}22`,borderRadius:'14px',padding:'0.7rem 0.3rem',display:'flex',flexDirection:'column',alignItems:'center',gap:'0.2rem',cursor:'pointer',WebkitTapHighlightColor:'transparent',transition:'all 0.15s',position:'relative',overflow:'hidden'}}>
              ...
              <span style={{fontSize:'0.6rem',fontWeight:700,color:'#F5EBD7',textAlign:'center',lineHeight:1.2}}>{a.label}</span>
            </button>
          ))}
        </div>
      </div>
      ...
      <div style={{marginBottom:'0.75rem'}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'0.45rem'}}>
          {[
            {icon:'📜',label:'Ferman',     page:'ferman',          color:'#C89B3C'},
            {icon:'🌳',label:'Yetenekler', page:'yetenek_agaci',   color:'#3E8C5A'},
            {icon:'📅',label:'Etkinlikler',page:'etkinlik_takvimi',color:'#C89B3C'},
            {icon:'🕌',label:'Osmanlı Günü',page:'osmanli_gunu',   color:'#C89B3C'},
      ...
      <div style={{background:'#2D1800',border:'1px solid rgba(245,235,215,0.08)',borderRadius:'10px',padding:'1rem',boxShadow:'none'}}>
        <div style={{fontFamily:"'Cinzel',serif",fontSize:'1rem',fontWeight:800,color:'#F5EBD7',marginBottom:'0.75rem'}}>Hızlı Erişim</div>
        <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'0.45rem'}}>
          {[
            ...
            <button key={i} onClick={() => onNavigate(a.page)}
              style={{background:'#1A0E00',border:'1px solid rgba(245,235,215,0.07)',borderRadius:'14px',padding:'0.7rem 0.3rem',display:'flex',flexDirection:'column',alignItems:'center',gap:'0.25rem',cursor:'pointer',WebkitTapHighlightColor:'transparent',transition:'all 0.15s'}}>
              <span style={{fontSize:'1.45rem',lineHeight:1}}>{a.icon}</span>
              <span style={{fontSize:'0.65rem',fontWeight:700,color:'#A9A6A0',textAlign:'center',lineHeight:1.2}}>{a.label}</span>
            </button>
          ))}
        </div>
      </div>
      ...
      <button onClick={()=>setSupportOpen(true)}
        style={{width:'100%',display:'flex',alignItems:'center',gap:'0.75rem',background:'linear-gradient(135deg,rgba(194,69,61,0.08),rgba(200,155,60,0.05))',border:'1px solid rgba(194,69,61,0.2)',borderRadius:'10px',padding:'0.85rem 1rem',cursor:'pointer',fontFamily:"'Inter',sans-serif",textAlign:'left',marginBottom:'0.75rem',WebkitTapHighlightColor:'transparent'}}>
        <span style={{fontSize:'1.5rem'}}>🆘</span>
        <div>
          <div style={{fontSize:'0.9rem',fontWeight:800,color:'#C2453D'}}>Destek Talebi</div>
          <div style={{fontSize:'0.7rem',color:'#A9A6A0'}}>Sorun mu var? Bize bildir, yardım edelim.</div>
        </div>
        <span style={{marginLeft:'auto',fontSize:'0.8rem',color:'#C2453D'}}>→</span>
      </button>
      ...
      {(()=>{
        ...
        return (
          <div style={{background:'rgba(96,165,250,0.06)',border:'1px solid rgba(96,165,250,0.25)',borderRadius:'10px',padding:'0.85rem 1rem',marginBottom:'0.75rem'}}>
            <div style={{fontSize:'0.78rem',fontWeight:800,color:'#C89B3C',marginBottom:'0.5rem'}}>💬 Destek Cevaplarınız</div>
            {myMsgs.slice(-3).map(m=>(
              <div key={m.id} style={{marginBottom:'0.5rem',paddingBottom:'0.5rem',borderBottom:'1px solid rgba(255,255,255,0.05)'}}>
                <div style={{fontSize:'0.72rem',color:'#A9A6A0',marginBottom:'0.2rem'}}>{new Date(m.ts).toLocaleDateString('tr-TR')} — {m.text.length>60?m.text.slice(0,60)+'…':m.text}</div>
                {m.replies.map((r,i)=>(
                  <div key={i} style={{background:'rgba(96,165,250,0.1)',border:'1px solid rgba(96,165,250,0.2)',borderRadius:'8px',padding:'0.45rem 0.6rem',fontSize:'0.8rem',color:'#E8C06A'}}>
                    🔑 {r.text}
                  </div>
                ))}
              </div>
      ...
      {supportOpen && (
        <div onClick={()=>setSupportOpen(false)} style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.65)',zIndex:3000,display:'flex',alignItems:'flex-end',justifyContent:'center'}}>
          <div onClick={e=>e.stopPropagation()} style={{background:'#2D1800',borderRadius:'24px 24px 0 0',padding:'1.5rem 1.25rem',width:'100%',maxWidth:'480px',animation:'slideUp 0.25s ease',boxShadow:'0 -8px 40px rgba(0,0,0,0.2)'}}>
            {supportSent ? (
              <div style={{textAlign:'center',padding:'1.5rem 0'}}>
                <div style={{fontSize:'2.5rem',marginBottom:'0.5rem'}}>✅</div>
                <div style={{fontSize:'1rem',fontWeight:800,color:'#4C9A6B'}}>Talebiniz İletildi!</div>
                <div style={{fontSize:'0.8rem',color:'#A9A6A0',marginTop:'0.35rem'}}>En kısa sürede cevap verilecek.</div>
              </div>
            ) : (
              <>
                <div style={{display:'flex',alignItems:'center',gap:'0.6rem',marginBottom:'1rem'}}>
                  <span style={{fontSize:'1.5rem'}}>🆘</span>
                  <div style={{fontSize:'1rem',fontWeight:800,color:'#F5EBD7'}}>Destek Talebi</div>
                  <button onClick={()=>setSupportOpen(false)} style={{marginLeft:'auto',background:'#F1F5F9',border:'none',borderRadius:'8px',padding:'0.3rem 0.65rem',cursor:'pointer',color:'#A9A6A0',fontSize:'0.85rem'}}>✕</button>
                </div>
                <div style={{fontSize:'0.8rem',color:'#A9A6A0',marginBottom:'0.6rem'}}>Kullanıcı: <strong>{profile?.username}</strong></div>
                <textarea value={supportText} onChange={e=>setSupportText(e.target.value)}
                  placeholder="Sorununuzu veya talebinizi yazın..."
                  rows={4}
                  style={{width:'100%',padding:'0.75rem',border:'1.5px solid rgba(200,155,60,0.25)',borderRadius:'12px',fontFamily:"'Inter',sans-serif",fontSize:'0.9rem',color:'#F5EBD7',resize:'none',outline:'none',boxSizing:'border-box',background:'#2D1800'}}
                />
                <button onClick={sendSupportMsg} disabled={!supportText.trim()}
                  style={{width:'100%',padding:'0.8rem',borderRadius:'14px',border:'none',background:supportText.trim()?'#C2453D':'#E2E8F0',color:supportText.trim()?'#fff':'#94A3B8',fontFamily:"'Inter',sans-serif",fontWeight:700,fontSize:'0.92rem',cursor:supportText.trim()?'pointer':'default',marginTop:'0.65rem',transition:'all 0.15s'}}>
      ...
      {annModal && (
        <div onClick={()=>setAnnModal(null)} style={{position:'fixed',inset:0,background:'rgba(17,21,28,0.9)',zIndex:2000,display:'flex',alignItems:'center',justifyContent:'center',padding:'1rem'}}>
          <div onClick={e=>e.stopPropagation()} style={{background:'#2D1800',borderRadius:'14px',padding:'1.5rem',width:'100%',maxWidth:'400px',animation:'slideUp 0.2s ease',boxShadow:'0 20px 60px rgba(0,0,0,0.3)'}}>
            <div style={{display:'flex',alignItems:'center',gap:'0.75rem',marginBottom:'1rem'}}>
              <span style={{fontSize:'2rem'}}>{annModal.icon||'📣'}</span>
              <div>
                <div style={{fontSize:'1rem',fontWeight:800,color:'#F5EBD7'}}>{annModal.title}</div>
                <div style={{fontSize:'0.72rem',color:catColor[annModal.category]||'#A9A6A0',fontWeight:700}}>{annModal.category} • {timeAgo(annModal.ts)}</div>
              </div>
            </div>
            <div style={{fontSize:'0.88rem',color:'#334155',lineHeight:'1.65',marginBottom:'1.25rem'}}>{annModal.body||annModal.text}</div>
            <button onClick={()=>setAnnModal(null)} style={{width:'100%',padding:'0.7rem',borderRadius:'12px',border:'none',background:'#C89B3C',color:'#F5EBD7',fontFamily:"'Inter',sans-serif",fontWeight:700,fontSize:'0.9rem',cursor:'pointer'}}>Kapat</button>
          </div>
        </div>
      )}
...
function AdminElectionTab({ elections_multi, setElections_multi, setMsg, cs, inp }) {
  ...
  return (
    <div>
      <div style={{fontSize:'0.7rem',color:'#A9A6A0',marginBottom:'0.65rem'}}>Her makam için bağımsız seçim yönetin. Parti liderleri aday gösterir, oylamayı siz başlatırsınız.</div>
          ...
          <div key={pos.key} style={{...cs,border:`1px solid ${el.active?'rgba(76,154,107,0.3)':'rgba(255,215,0,0.15)'}`,marginBottom:'0.5rem'}}>
            <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'0.5rem'}}>
              <div style={{display:'flex',alignItems:'center',gap:'0.5rem'}}>
                ...
                <div>
                  <div style={{fontWeight:800,color:'#C89B3C',fontSize:'0.85rem'}}>{pos.title}</div>
                  <div style={{fontSize:'0.65rem',color:'#A9A6A0'}}>
                    {cands.length} aday •
                    {el.active ? <span style={{color:'#4C9A6B'}}> 🟢 Aktif</span> : el.winner ? <span style={{color:'#C89B3C'}}> 🏆 {el.winner}</span> : <span style={{color:'#A9A6A0'}}> ⏸ Pasif</span>}
                  </div>
                </div>
              </div>
              <div style={{display:'flex',gap:'0.3rem',flexWrap:'wrap',justifyContent:'flex-end'}}>
                {!el.active
                  ...
                  : <button onClick={()=>endPos(pos.key)} style={{padding:'0.35rem 0.65rem',borderRadius:'8px',border:'1px solid rgba(200,155,60,0.3)',background:'rgba(200,155,60,0.08)',color:'#C89B3C',cursor:'pointer',fontWeight:700,fontSize:'0.72rem',minHeight:32}}>🏁 Bitir</button>}
                <button onClick={()=>resetPos(pos.key)} style={{padding:'0.35rem 0.55rem',borderRadius:'8px',border:'1px solid rgba(194,69,61,0.25)',background:'rgba(194,69,61,0.08)',color:'#E08C87',cursor:'pointer',fontWeight:700,fontSize:'0.72rem',minHeight:32}}>↺</button>
              </div>
            </div>
            {PARTY_BASED_KEYS.includes(pos.key) ? (
              <div style={{marginBottom:'0.4rem'}}>
                <div style={{fontSize:'0.65rem',color:'#C89B3C',marginBottom:'0.35rem',fontWeight:700}}>⚑ Parti Bazlı Seçim — İlk 10 Parti Otomatik Aday</div>
                <button onClick={()=>loadPartyCandidates(pos.key)} style={{padding:'0.38rem 0.75rem',borderRadius:'8px',border:'1px solid rgba(200,155,60,0.35)',background:'rgba(167,139,250,0.1)',color:'#C89B3C',cursor:'pointer',fontWeight:700,fontSize:'0.73rem',minHeight:32}}>🔄 Parti Liderlerini Yükle</button>
              </div>
            ) : (
              <div style={{display:'flex',gap:'0.35rem',marginBottom:'0.4rem'}}>
                ...
                <button onClick={()=>addCand(pos.key)} style={{padding:'0.38rem 0.65rem',borderRadius:'8px',border:'1px solid rgba(255,215,0,0.4)',background:'rgba(255,215,0,0.1)',color:'#C89B3C',cursor:'pointer',fontWeight:700,fontSize:'0.75rem',whiteSpace:'nowrap',minHeight:34}}>+ Ekle</button>
              </div>
            )}
            {cands.length>0 && (
              <div style={{display:'flex',flexWrap:'wrap',gap:'0.3rem'}}>
                {sorted.map((c,i)=>{
                  ...
                  return (
                    <div key={c.username} style={{background:'rgba(245,235,215,0.04)',border:`1px solid ${i===0&&totalV>0?'rgba(200,155,60,0.4)':'rgba(255,255,255,0.08)'}`,borderRadius:'8px',padding:'3px 8px',display:'flex',alignItems:'center',gap:'0.3rem',fontSize:'0.72rem'}}>
                      <span style={{color:i===0&&totalV>0?'#C89B3C':'#94A3B8'}}>{i===0&&totalV>0?'🥇':i===1&&totalV>0?'🥈':i===2&&totalV>0?'🥉':'·'} {c.username}</span>
                      {totalV>0&&<span style={{color:'#A9A6A0',fontSize:'0.6rem'}}>({votes[c.username]||0} oy · {pct}%)</span>}
                      {!el.active&&<span onClick={()=>removeCand(pos.key,c.username)} style={{cursor:'pointer',color:'#C2453D',fontSize:'0.85rem',lineHeight:1,marginLeft:'0.15rem'}}>×</span>}
                    </div>
                  );
                })}
              </div>
...
function AdminMakamlarTab({ allUsers, setAllUsersRaw, setMsg, cs, inp }) {
  ...
  return (
    <div>
      <div style={{fontSize:'0.7rem',color:'#A9A6A0',marginBottom:'0.65rem'}}>Kullanıcıları makama direkt atayın. Seçim kazandıklarında da buraya yansır.</div>
        ...
        return (
          <div key={m.key} style={{...cs,border:`1px solid ${current?'rgba(255,215,0,0.2)':'rgba(255,255,255,0.06)'}`,marginBottom:'0.45rem'}}>
            <div style={{display:'flex',alignItems:'center',gap:'0.5rem',marginBottom:'0.4rem'}}>
              ...
              <div style={{flex:1}}>
                <div style={{fontWeight:700,color:'#C89B3C',fontSize:'0.82rem'}}>{m.title}</div>
                <div style={{fontSize:'0.67rem',color:current?'#4C9A6B':'#3B4E63'}}>{current?`👤 ${current}`:'Boş — Atanmamış'}</div>
              </div>
              {current&&<button onClick={()=>remove(m.title)} style={{padding:'0.25rem 0.5rem',borderRadius:'6px',border:'1px solid rgba(194,69,61,0.3)',background:'rgba(194,69,61,0.08)',color:'#E08C87',cursor:'pointer',fontSize:'0.68rem',fontWeight:700,minHeight:28}}>✕</button>}
            </div>
            <div style={{display:'flex',gap:'0.35rem'}}>
              ...
              <button onClick={()=>assign(m.title)} style={{padding:'0.38rem 0.7rem',borderRadius:'8px',border:'1px solid rgba(255,215,0,0.4)',background:'rgba(255,215,0,0.1)',color:'#C89B3C',cursor:'pointer',fontWeight:700,fontSize:'0.75rem',minHeight:34}}>Ata</button>
            </div>
          </div>
        );
      })}
      <div style={{...cs,borderColor:'rgba(200,155,60,0.25)',marginTop:'0.75rem'}}>
        <div style={{fontWeight:800,color:'#C89B3C',fontSize:'0.82rem',marginBottom:'0.6rem'}}>💎 VIP & Paket Ver</div>
        {[...allUsers].filter(u=>!u.isBot).slice(0,20).map(u=>(
          <div key={u.id} style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'0.38rem 0',borderBottom:'1px solid rgba(255,255,255,0.04)'}}>
            <div style={{fontSize:'0.78rem',color:'#F5EBD7',fontWeight:600}}>{u.username}</div>
            <div style={{display:'flex',gap:'0.3rem'}}>
              ...
              <button onClick={()=>giveEdu(u)} style={{padding:'0.2rem 0.5rem',borderRadius:'6px',border:'1px solid rgba(200,155,60,0.35)',background:'rgba(200,155,60,0.1)',color:'#C89B3C',cursor:'pointer',fontSize:'0.65rem',fontWeight:700}}>📚 Edu</button>
            </div>
          </div>
        ))}
      </div>
  ...
  return (
    <div>
      <div style={{...cs,display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'0.65rem'}}>
        <div>
          <div style={{fontWeight:800,color:'#F5EBD7',fontSize:'0.9rem'}}>💬 Destek Mesajları</div>
          <div style={{fontSize:'0.7rem',color:'#A9A6A0',marginTop:'0.2rem'}}>{supportMsgs.length} mesaj • {supportMsgs.filter(m=>m.status==='pending').length} yanıtsız</div>
        </div>
        <button onClick={refresh} style={{padding:'0.3rem 0.65rem',borderRadius:'8px',border:'1px solid rgba(245,235,215,0.1)',background:'rgba(245,235,215,0.03)',color:'#A9A6A0',fontSize:'0.72rem',cursor:'pointer',fontFamily:"'Inter',sans-serif",fontWeight:700}}>🔄 Yenile</button>
      </div>
      {supportMsgs.length === 0 && <div style={{...cs,textAlign:'center',color:'#A9A6A0',padding:'2rem',fontSize:'0.85rem'}}>Henüz destek mesajı yok</div>}
      {[...supportMsgs].reverse().map(m => (
        <div key={m.id} style={{...cs,border:`1px solid ${m.status==='pending'?'rgba(194,69,61,0.25)':'rgba(76,154,107,0.2)'}`,marginBottom:'0.5rem'}}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:'0.5rem'}}>
            <div>
              <div style={{fontWeight:700,color:'#F5EBD7',fontSize:'0.85rem'}}>👤 {m.from||'Anonim'}</div>
              <div style={{fontSize:'0.65rem',color:'#A9A6A0'}}>{new Date(m.ts).toLocaleString('tr-TR')} • <span style={{color:m.status==='pending'?'#C2453D':'#4C9A6B',fontWeight:700}}>{m.status==='pending'?'⏳ Yanıtsız':'✅ Yanıtlandı'}</span></div>
            </div>
            <button onClick={()=>deleteMsg(m.id)} style={{padding:'0.2rem 0.5rem',borderRadius:'6px',border:'1px solid rgba(194,69,61,0.25)',background:'rgba(194,69,61,0.08)',color:'#C2453D',fontSize:'0.7rem',cursor:'pointer',fontFamily:"'Inter',sans-serif"}}>🗑️</button>
          </div>
          <div style={{background:'rgba(245,235,215,0.02)',borderRadius:'8px',padding:'0.6rem 0.75rem',fontSize:'0.82rem',color:'#C8D3DC',marginBottom:'0.5rem',lineHeight:1.5}}>{m.text}</div>
          {(m.replies||[]).map((r,i)=>(
          ...
          <div style={{display:'flex',gap:'0.4rem',marginTop:'0.4rem'}}>
            ...
            <button onClick={()=>sendReply(m.id)} style={{padding:'0.4rem 0.75rem',borderRadius:'8px',border:'none',background:'#4C9A6B',color:'#F5EBD7',fontFamily:"'Inter',sans-serif",fontWeight:700,fontSize:'0.75rem',cursor:'pointer',whiteSpace:'nowrap'}}>Yanıtla</button>
          </div>
        </div>
      ))}
    </div>
...
function AdminPushTab({ setMsg, cs, inp }) {
  ...
  return (
    <div style={cs}>
      <div style={{fontWeight:700,color:'#F5EBD7',marginBottom:'0.75rem',fontSize:'0.9rem'}}>📲 Push Bildirim Yayını</div>

      <div style={{display:'flex',gap:'0.5rem',marginBottom:'0.65rem'}}>
        {[['all','🌍 Tüm Kullanıcılar'],['user','👤 Belirli Kullanıcı']].map(([v,l])=>(
          <button key={v} onClick={()=>setPushMode(v)}
            style={{flex:1,padding:'0.45rem',borderRadius:'8px',border:'1px solid '+(pushMode===v?'rgba(194,69,61,0.4)':'rgba(255,255,255,0.08)'),background:pushMode===v?'rgba(194,69,61,0.12)':'rgba(255,255,255,0.03)',color:pushMode===v?'#E08C87':'#A9A6A0',fontFamily:"'Inter',sans-serif",fontWeight:700,fontSize:'0.75rem',cursor:'pointer'}}>
            {l}
          </button>
        ))}
      </div>
      ...
      <button onClick={sendPush} disabled={pushLoading}
        style={{width:'100%',padding:'0.75rem',borderRadius:'10px',border:'none',background:pushLoading?'rgba(194,69,61,0.3)':'rgba(194,69,61,0.85)',color:'#F5EBD7',fontFamily:"'Inter',sans-serif",fontWeight:700,fontSize:'0.9rem',cursor:pushLoading?'not-allowed':'pointer'}}>
...
function AdminPage({ profile, showNotif, onNavigate }) {
  ...
  const cs = {background:'rgba(245,235,215,0.03)',borderRadius:'14px',padding:'1rem',border:'1px solid rgba(245,235,215,0.08)',marginBottom:'0.65rem'};
  const inp = {width:'100%',background:'rgba(245,235,215,0.03)',border:'1px solid rgba(245,235,215,0.1)',borderRadius:'10px',padding:'0.55rem 0.8rem',color:'#F5EBD7',fontFamily:"'Inter',sans-serif",fontSize:'15px',outline:'none',boxSizing:'border-box'};
  ...
  return (
    <div style={{padding:'0.7rem',minHeight:'100%',background:'rgba(18,10,0,0.99)'}}>
      <div style={{background:'linear-gradient(135deg,rgba(194,69,61,0.15),rgba(26,14,0,0.95))',border:'1px solid rgba(194,69,61,0.25)',borderRadius:'10px',padding:'1rem 1.25rem',marginBottom:'0.75rem'}}>
        ...
        <div style={{fontSize:'1.1rem',fontWeight:900,color:'#F5EBD7',fontFamily:"'Cinzel',serif"}}>Admin: {profile?.username}</div>
        <div style={{fontSize:'0.7rem',color:'#A9A6A0',marginTop:'0.1rem'}}>{allUsers.length} kullanıcı • {onlineCnt} online • {bannedCount} banlı</div>
      </div>
      ...
      <div style={{display:'flex',gap:'4px',overflowX:'auto',scrollbarWidth:'none',marginBottom:'0.75rem'}}>
        {tabs.map(([id,label]) => (
          <button key={id} onClick={()=>{setTab(id);if(id!=='manage')setSelectedUser(null);}}
            style={{padding:'0.38rem 0.7rem',borderRadius:'8px',border:`1px solid ${tab===id?'rgba(194,69,61,0.4)':'rgba(255,255,255,0.07)'}`,background:tab===id?'rgba(194,69,61,0.12)':'rgba(255,255,255,0.03)',color:tab===id?'#E08C87':'#A9A6A0',fontFamily:"'Inter',sans-serif",fontWeight:700,fontSize:'0.72rem',cursor:'pointer',whiteSpace:'nowrap',flexShrink:0}}>
            {label}
          </button>
        ))}
      </div>
      ...
      {msg && (
        <div style={{background:'rgba(76,154,107,0.08)',border:'1px solid rgba(76,154,107,0.2)',borderRadius:'10px',padding:'0.55rem 0.8rem',fontSize:'0.78rem',color:'#4C9A6B',marginBottom:'0.65rem',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
          <span>{msg}</span>
          <button onClick={()=>setMsg('')} style={{background:'none',border:'none',color:'#A9A6A0',cursor:'pointer',fontSize:'1rem',lineHeight:1}}>✕</button>
        </div>
      )}
      ...
      {tab==='dashboard' && (
        <div>
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'0.5rem',marginBottom:'0.65rem'}}>
            {[
              ['👥','Toplam Kullanıcı', allUsers.length, '#C89B3C'],
              ['🟢','Şu An Online', onlineCnt, '#4C9A6B'],
              ['🚫','Banlı', bannedCount, '#C2453D'],
              ['⭐','Admin', adminCount, '#C89B3C'],
              ...
              <div key={lbl} style={{...cs,textAlign:'center',padding:'0.75rem'}}>
                ...
                <div style={{fontSize:'0.6rem',color:'#A9A6A0',fontWeight:700,textTransform:'uppercase'}}>{lbl}</div>
              </div>
            ))}
          </div>

          <div style={cs}>
            <div style={{fontWeight:800,color:'#F5EBD7',marginBottom:'0.65rem',fontSize:'0.85rem'}}>🏆 En Zengin Oyuncular</div>
            {[...allUsers].sort((a,b)=>(b.money||0)-(a.money||0)).slice(0,5).map((u,i)=>(
              <div key={u.id} style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'0.4rem 0',borderBottom:'1px solid rgba(255,255,255,0.04)'}}>
                <div style={{display:'flex',alignItems:'center',gap:'0.5rem'}}>
                  ...
                  <div>
                    <div style={{fontSize:'0.8rem',fontWeight:700,color:u.banned?'#C2453D':'#F5EBD7'}}>{u.username} {u.banned?'🚫':''}{u.role==='admin'?'⭐':''}</div>
                    <div style={{fontSize:'0.65rem',color:'#A9A6A0'}}>Lv.{u.level||1} • {u.city||'?'}</div>
                  </div>
                </div>
                <div style={{color:'#4C9A6B',fontWeight:800,fontSize:'0.82rem'}}>{fmtM(u.money||0)}</div>
              </div>
            ))}
            {allUsers.length===0 && <div style={{color:'#A9A6A0',fontSize:'0.8rem',textAlign:'center',padding:'1rem'}}>Henüz kayıtlı kullanıcı yok</div>}
          </div>

          <div style={cs}>
            <div style={{fontWeight:800,color:'#F5EBD7',marginBottom:'0.65rem',fontSize:'0.85rem'}}>⚡ Hızlı Erişim</div>
            <div style={{display:'flex',flexWrap:'wrap',gap:'0.4rem'}}>
              {[['👥','Kullanıcılar','users'],['🛡️','Yönet','manage'],['📢','Duyuru','announce'],['📋','Loglar','logs'],['🛠️','Araçlar','tools']].map(([ic,lbl,t])=>(
                <button key={t} onClick={()=>setTab(t)} style={{padding:'0.4rem 0.75rem',borderRadius:'8px',border:'1px solid rgba(245,235,215,0.1)',background:'rgba(245,235,215,0.03)',color:'#A9A6A0',fontFamily:"'Inter',sans-serif",fontWeight:700,fontSize:'0.75rem',cursor:'pointer'}}>{ic} {lbl}</button>
              ))}
            </div>
          </div>
        </div>
      ...
      {tab==='users' && (
        <div>
          <div style={{display:'flex',gap:'0.5rem',marginBottom:'0.65rem'}}>
            ...
            <button onClick={refreshUsers} style={{padding:'0.55rem 0.75rem',borderRadius:'10px',border:'1px solid rgba(245,235,215,0.1)',background:'rgba(245,235,215,0.03)',color:'#A9A6A0',cursor:'pointer',fontWeight:700,fontSize:'0.8rem',whiteSpace:'nowrap'}}>↻</button>
          </div>
          <div style={{fontSize:'0.7rem',color:'#A9A6A0',marginBottom:'0.5rem'}}>{filteredUsers.length} kullanıcı gösteriliyor</div>
          {filteredUsers.map(u => (
            <div key={u.id} style={{...cs,marginBottom:'0.4rem',padding:'0.75rem',border:`1px solid ${u.banned?'rgba(194,69,61,0.2)':'rgba(255,255,255,0.06)'}`,cursor:'pointer'}}
              onClick={()=>{setSelectedUser(u);setTab('manage');}}>
              <div style={{display:'flex',alignItems:'center',justifyContent:'space-between'}}>
                <div style={{display:'flex',alignItems:'center',gap:'0.6rem'}}>
                  <div style={{width:'32px',height:'32px',borderRadius:'50%',background:u.banned?'rgba(194,69,61,0.2)':'rgba(200,155,60,0.2)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'0.9rem',flexShrink:0}}>
                    {u.role==='admin'?'⭐':u.banned?'🚫':'👤'}
                  </div>
                  <div>
                    <div style={{fontSize:'0.85rem',fontWeight:700,color:u.banned?'#E08C87':u.role==='admin'?'#C89B3C':'#F5EBD7'}}>{u.username}</div>
                    <div style={{fontSize:'0.65rem',color:'#A9A6A0'}}>{u.email} • Lv.{u.level||1} • {u.city||'?'}</div>
                  </div>
                </div>
                <div style={{textAlign:'right',flexShrink:0}}>
                  <div style={{color:'#4C9A6B',fontWeight:700,fontSize:'0.82rem'}}>{fmtM(u.money||0)}</div>
                  <div style={{fontSize:'0.6rem',color:'#A9A6A0'}}>{u.underCoin||0} Altın</div>
                </div>
              </div>
            </div>
          ))}
          {filteredUsers.length===0 && <div style={{...cs,textAlign:'center',color:'#A9A6A0',padding:'2rem'}}>Kullanıcı bulunamadı</div>}
        </div>
      )}
      ...
      {tab==='manage' && (
        <div>
          {!selectedUser ? (
            <div>
              <div style={{color:'#A9A6A0',fontSize:'0.8rem',marginBottom:'0.65rem'}}>Yönetmek için Kullanıcılar sekmesinden bir kullanıcı seç</div>
              <div style={{display:'flex',flexWrap:'wrap',gap:'0.4rem'}}>
                {allUsers.slice(0,10).map(u => (
                  <button key={u.id} onClick={()=>setSelectedUser(u)}
                    style={{padding:'0.35rem 0.75rem',borderRadius:'8px',border:'1px solid rgba(245,235,215,0.1)',background:'rgba(245,235,215,0.03)',color:'#A9A6A0',fontFamily:"'Inter',sans-serif",fontSize:'0.78rem',cursor:'pointer',fontWeight:600}}>
            ...
            <div>
              <div style={{...cs,background:'linear-gradient(135deg,rgba(200,155,60,0.08),rgba(26,14,0,0.9))'}}>
                <div style={{display:'flex',alignItems:'center',gap:'0.75rem',marginBottom:'0.75rem'}}>
                  <div style={{width:'44px',height:'44px',borderRadius:'50%',background:selectedUser.banned?'rgba(194,69,61,0.2)':'rgba(200,155,60,0.2)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'1.2rem',flexShrink:0}}>
                    {selectedUser.role==='admin'?'⭐':selectedUser.banned?'🚫':'👤'}
                  </div>
                  <div style={{flex:1}}>
                    <div style={{fontWeight:900,color:'#F5EBD7',fontSize:'1rem'}}>{selectedUser.username}</div>
                    <div style={{fontSize:'0.7rem',color:'#A9A6A0'}}>{selectedUser.email} • {selectedUser.city||'?'} • Lv.{selectedUser.level||1}</div>
                    <div style={{fontSize:'0.7rem',marginTop:'0.15rem'}}>
                      {selectedUser.banned && <span style={{color:'#C2453D',fontWeight:700}}>🚫 Banlı: {selectedUser.banReason}</span>}
                      {selectedUser.role==='admin' && <span style={{color:'#C89B3C',fontWeight:700}}>⭐ Admin</span>}
                    </div>
                  </div>
                  <button onClick={()=>setSelectedUser(null)} style={{background:'rgba(245,235,215,0.05)',border:'1px solid rgba(245,235,215,0.1)',borderRadius:'8px',padding:'0.3rem 0.6rem',color:'#A9A6A0',cursor:'pointer',fontWeight:700,fontSize:'0.78rem'}}>✕</button>
                </div>
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:'0.4rem',marginBottom:'0.75rem'}}>
                  {[['💰',fmtM(selectedUser.money||0),'Para'],['💎',selectedUser.underCoin||0,'Altın'],['⭐',selectedUser.level||1,'Seviye'],['📊',selectedUser.xp||0,'XP'],['❤️',selectedUser.hp||100,'HP'],['🏙️',selectedUser.city||'?','Şehir']].map(([ic,v,l])=>(
                    <div key={l} style={{background:'rgba(245,235,215,0.02)',borderRadius:'8px',padding:'0.4rem',textAlign:'center'}}>
                      <div style={{fontSize:'0.9rem'}}>{ic}</div>
                      <div style={{fontWeight:700,color:'#F5EBD7',fontSize:'0.75rem'}}>{v}</div>
                      <div style={{fontSize:'0.55rem',color:'#A9A6A0',textTransform:'uppercase'}}>{l}</div>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div style={cs}>
                ...
                <div style={{display:'flex',gap:'0.4rem',marginBottom:'0.4rem'}}>
                  ...
                  <button onClick={()=>giveMoney(selectedUser)} style={{padding:'0.55rem 0.75rem',borderRadius:'10px',border:'none',background:'#4C9A6B',color:'#F5EBD7',fontWeight:700,fontSize:'0.78rem',cursor:'pointer',whiteSpace:'nowrap'}}>+ Ver</button>
                </div>
                <div style={{display:'flex',gap:'0.4rem',marginBottom:'0.4rem'}}>
                  ...
                  <button onClick={()=>setMoneyDirect(selectedUser)} style={{padding:'0.55rem 0.75rem',borderRadius:'10px',border:'none',background:'#C89B3C',color:'#000',fontWeight:700,fontSize:'0.78rem',cursor:'pointer',whiteSpace:'nowrap'}}>Ayarla</button>
                </div>
                <div style={{display:'flex',gap:'0.4rem',flexWrap:'wrap'}}>
                  {[1000,5000,10000,50000,100000,1000000].map(n=>(
                    <button key={n} onClick={()=>{setGiftAmount(String(n));}} style={{padding:'0.25rem 0.55rem',borderRadius:'7px',border:'1px solid rgba(245,235,215,0.08)',background:'rgba(245,235,215,0.02)',color:'#A9A6A0',fontSize:'0.68rem',cursor:'pointer',fontWeight:700}}>{fmtM(n)}</button>
                  ))}
                </div>
              </div>
              ...
              {/* Altın ver */}
              <div style={cs}>
                <div style={{fontWeight:700,color:'#C89B3C',marginBottom:'0.5rem',fontSize:'0.8rem'}}>💎 Altın İşlemleri</div>
                <div style={{display:'flex',gap:'0.4rem'}}>
                  <input type="number" value={giftUC} onChange={e=>setGiftUC(e.target.value)} placeholder="Verilecek Altın" style={{...inp,flex:1}} />
                  <button onClick={()=>giveUC(selectedUser)} style={{padding:'0.55rem 0.75rem',borderRadius:'10px',border:'none',background:'#C89B3C',color:'#F5EBD7',fontWeight:700,fontSize:'0.78rem',cursor:'pointer',whiteSpace:'nowrap'}}>+ Ver</button>
                </div>
              </div>

              {/* Ban işlemleri */}
              <div style={cs}>
                <div style={{fontWeight:700,color:'#C2453D',marginBottom:'0.5rem',fontSize:'0.8rem'}}>🚫 Ban İşlemleri</div>
                {!selectedUser.banned && (
                ...
                <div style={{display:'flex',gap:'0.4rem',flexWrap:'wrap'}}>
                  <button onClick={()=>banToggle(selectedUser)}
                    style={{padding:'0.45rem 0.85rem',borderRadius:'10px',border:'none',background:selectedUser.banned?'#4C9A6B':'#C2453D',color:'#F5EBD7',fontWeight:700,fontSize:'0.78rem',cursor:'pointer'}}>
                    {selectedUser.banned ? '✅ Banı Kaldır' : '🚫 Banla'}
                  </button>
                  <button onClick={()=>makeAdmin(selectedUser)}
                    style={{padding:'0.45rem 0.85rem',borderRadius:'10px',border:'none',background:selectedUser.role==='admin'?'#64748B':'#C89B3C',color:selectedUser.role==='admin'?'#fff':'#000',fontWeight:700,fontSize:'0.78rem',cursor:'pointer'}}>
                    {selectedUser.role==='admin' ? '↓ Admin Al' : '⭐ Admin Yap'}
                  </button>
                  <button onClick={()=>resetUser(selectedUser)}
                    style={{padding:'0.45rem 0.85rem',borderRadius:'10px',border:'1px solid rgba(200,155,60,0.25)',background:'rgba(200,155,60,0.08)',color:'#C89B3C',fontWeight:700,fontSize:'0.78rem',cursor:'pointer'}}>
                    ↺ Sıfırla
                  </button>
                  <button onClick={()=>giveEduPackage(selectedUser)}
                    style={{padding:'0.45rem 0.85rem',borderRadius:'10px',border:'1px solid rgba(200,155,60,0.35)',background:'rgba(200,155,60,0.15)',color:'#C89B3C',fontWeight:700,fontSize:'0.78rem',cursor:'pointer'}}>
                    🎓 Edu Paketi
                  </button>
                  ...
                  <button onClick={()=>{if(window.confirm('Kullanıcıyı sil?'))deleteUser(selectedUser);}}
                    style={{padding:'0.45rem 0.85rem',borderRadius:'10px',border:'1px solid rgba(194,69,61,0.25)',background:'rgba(194,69,61,0.08)',color:'#C2453D',fontWeight:700,fontSize:'0.78rem',cursor:'pointer'}}>
                    🗑️ Sil
                  </button>
                </div>
              </div>
      ...
      {tab==='announce' && (
        <div>
          <div style={cs}>
            <div style={{fontWeight:800,color:'#F5EBD7',marginBottom:'0.75rem',fontSize:'0.85rem'}}>📢 Sistem Duyurusu Yayınla</div>
            <div style={{marginBottom:'0.5rem'}}>
              <div style={{fontSize:'0.7rem',color:'#A9A6A0',fontWeight:700,marginBottom:'0.3rem'}}>Başlık *</div>
              <input value={annTitle} onChange={e=>setAnnTitle(e.target.value)} placeholder="Duyuru başlığı..."
                style={{...inp,marginBottom:0}} />
            </div>
            <div style={{marginBottom:'0.5rem'}}>
              <div style={{fontSize:'0.7rem',color:'#A9A6A0',fontWeight:700,marginBottom:'0.3rem'}}>Kategori & İkon</div>
              <div style={{display:'grid',gridTemplateColumns:'1fr auto',gap:'0.4rem'}}>
                <select value={annCategory} onChange={e=>setAnnCategory(e.target.value)}
                  style={{...inp,marginBottom:0,background:'rgba(245,235,215,0.03)',color:'#F5EBD7'}}>
                  {['Sistem','Siyaset','Ekonomi','Hukuk','Etkinlik','Güvenlik','Eğitim'].map(c=>(
                    <option key={c} value={c} style={{background:'#1A0E00'}}>{c}</option>
                  ))}
                </select>
                <select value={annIcon} onChange={e=>setAnnIcon(e.target.value)}
                  style={{...inp,marginBottom:0,width:'60px',background:'rgba(245,235,215,0.03)',color:'#F5EBD7',textAlign:'center'}}>
                  {['📢','🏛️','💰','⚖️','🎉','🚔','🎓','⚠️','🔥','📌'].map(ic=>(
                    <option key={ic} value={ic} style={{background:'#1A0E00'}}>{ic}</option>
                  ))}
                </select>
              </div>
            </div>
            <div style={{marginBottom:'0.5rem'}}>
              <div style={{fontSize:'0.7rem',color:'#A9A6A0',fontWeight:700,marginBottom:'0.3rem'}}>Duyuru Metni *</div>
            ...
            <div style={{marginBottom:'0.65rem'}}>
              <div style={{fontSize:'0.7rem',color:'#A9A6A0',fontWeight:700,marginBottom:'0.3rem'}}>Görsel URL (isteğe bağlı)</div>
            ...
            <button onClick={sendAnnouncement} style={{width:'100%',padding:'0.7rem',borderRadius:'10px',border:'none',background:'linear-gradient(135deg,#C89B3C,#C89B3C)',color:'#F5EBD7',fontWeight:700,fontSize:'0.85rem',cursor:'pointer'}}>
              {annIcon} Duyuruyu Yayınla
            </button>
          </div>

          <div style={cs}>
            <div style={{fontWeight:700,color:'#F5EBD7',marginBottom:'0.65rem',fontSize:'0.85rem'}}>📋 Son Duyurular ({announcements.length})</div>
            {announcements.length===0 && <div style={{color:'#A9A6A0',fontSize:'0.8rem',textAlign:'center',padding:'1rem'}}>Henüz duyuru yok</div>}
            {announcements.map(a => (
              <div key={a.id} style={{padding:'0.6rem',borderBottom:'1px solid rgba(255,255,255,0.04)',borderRadius:'8px',marginBottom:'0.3rem'}}>
                ...
                <div style={{display:'flex',alignItems:'center',gap:'0.4rem',marginBottom:'0.2rem'}}>
                  ...
                  <span style={{fontSize:'0.82rem',fontWeight:800,color:'#F5EBD7'}}>{a.title||a.text}</span>
                  {a.category && <span style={{fontSize:'0.55rem',padding:'0.1rem 0.4rem',borderRadius:'6px',background:'rgba(200,155,60,0.14)',color:'#C89B3C',fontWeight:700}}>{a.category}</span>}
                </div>
                {a.title && <div style={{fontSize:'0.78rem',color:'#A9A6A0',marginBottom:'0.2rem'}}>{a.body||a.text}</div>}
                <div style={{fontSize:'0.62rem',color:'#A9A6A0'}}>{a.by} • {timeAgo(a.ts)}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div>
          <div style={{display:'flex',gap:'4px',marginBottom:'0.65rem',overflowX:'auto',scrollbarWidth:'none'}}>
            {[['all','Tümü'],['banned','Banlılar'],['admin','Adminler'],['rich','En Zengin']].map(([id,lbl])=>(
              <button key={id} onClick={()=>setTabLog(id)}
                style={{padding:'0.3rem 0.65rem',borderRadius:'7px',border:`1px solid ${tabLog===id?'rgba(194,69,61,0.4)':'rgba(255,255,255,0.07)'}`,background:tabLog===id?'rgba(194,69,61,0.1)':'rgba(255,255,255,0.03)',color:tabLog===id?'#E08C87':'#A9A6A0',fontFamily:"'Inter',sans-serif",fontWeight:700,fontSize:'0.72rem',cursor:'pointer',whiteSpace:'nowrap',flexShrink:0}}>
                {lbl}
              </button>
            ))}
          </div>
          {(tabLog==='all'?allUsers:tabLog==='banned'?allUsers.filter(u=>u.banned):tabLog==='admin'?allUsers.filter(u=>u.role==='admin'):[...allUsers].sort((a,b)=>(b.money||0)-(a.money||0)).slice(0,20))
            ...
            <div key={u.id} style={{...cs,marginBottom:'0.35rem',padding:'0.65rem',cursor:'pointer'}} onClick={()=>{setSelectedUser(u);setTab('manage');}}>
              <div style={{display:'flex',alignItems:'center',justifyContent:'space-between'}}>
                <div>
                  <div style={{fontSize:'0.82rem',fontWeight:700,color:u.banned?'#E08C87':u.role==='admin'?'#C89B3C':'#F5EBD7'}}>{u.role==='admin'?'⭐ ':u.banned?'🚫 ':''}{u.username}</div>
                  <div style={{fontSize:'0.65rem',color:'#A9A6A0'}}>{u.email||'—'} • {u.city||'?'} • Lv.{u.level||1}</div>
                </div>
                <div style={{textAlign:'right'}}>
          ...
          {allUsers.length===0 && <div style={{...cs,textAlign:'center',color:'#A9A6A0',padding:'2rem'}}>Henüz kullanıcı yok</div>}
        </div>
      )}
      ...
      {tab==='economy' && (
        <div>
          <div style={cs}>
            <div style={{fontWeight:800,color:'#F5EBD7',marginBottom:'0.65rem',fontSize:'0.85rem'}}>💰 Ekonomi Özeti</div>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'0.5rem',marginBottom:'0.65rem'}}>
              {[
                ['💰','Toplam Para', fmtM(totalMoney), '#4C9A6B'],
                ['📊','Ort. Bakiye', fmtM(allUsers.length>0?Math.floor(totalMoney/allUsers.length):0), '#C89B3C'],
                ['💎','Toplam VIP', allUsers.filter(u=>u.premium).length, '#C89B3C'],
                ['🪙','Toplam Altın', allUsers.reduce((s,u)=>s+(u.underCoin||0),0).toLocaleString('tr-TR'), '#C89B3C'],
              ].map(([ic,lbl,val,c]) => (
                <div key={lbl} style={{...cs,textAlign:'center',padding:'0.75rem',marginBottom:0}}>
                  ...
                  <div style={{fontSize:'0.58rem',color:'#A9A6A0',fontWeight:700,textTransform:'uppercase'}}>{lbl}</div>
                </div>
              ))}
            </div>
            <div style={{fontWeight:700,color:'#F5EBD7',marginBottom:'0.5rem',fontSize:'0.82rem'}}>💎 VIP Üyeler</div>
            {allUsers.filter(u=>u.premium).length === 0
              ? <div style={{color:'#A9A6A0',fontSize:'0.78rem',textAlign:'center',padding:'0.75rem'}}>Henüz VIP üye yok</div>
              : allUsers.filter(u=>u.premium).map(u=>(
                <div key={u.id} style={{display:'flex',justifyContent:'space-between',padding:'0.35rem 0',borderBottom:'1px solid rgba(255,255,255,0.04)',fontSize:'0.78rem'}}>
                  <span style={{color:'#C89B3C',fontWeight:700}}>💎 {u.username}</span>
                  <span style={{color:'#A9A6A0'}}>{fmtM(u.money||0)}</span>
                </div>
              ))
            }
          </div>
          <div style={cs}>
            <div style={{fontWeight:800,color:'#F5EBD7',marginBottom:'0.65rem',fontSize:'0.85rem'}}>🏢 Holdingler</div>
            {(() => {
              try {
                ...
                return (
                  <div>
                    <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'0.5rem'}}>
                      {[['🏢','Toplam Şirket',totalHoldings,'#C89B3C'],['💰','Toplam Değer',fmtM(totalAssets),'#4C9A6B']].map(([ic,lbl,val,c])=>(
                        <div key={lbl} style={{background:'rgba(245,235,215,0.02)',borderRadius:'10px',padding:'0.65rem',textAlign:'center'}}>
                          <div style={{fontSize:'1.1rem'}}>{ic}</div>
                          <div style={{fontWeight:800,color:c}}>{val}</div>
                          <div style={{fontSize:'0.6rem',color:'#A9A6A0',textTransform:'uppercase'}}>{lbl}</div>
                        </div>
                      ))}
                    </div>
                  </div>
                );
              } catch { return <div style={{color:'#A9A6A0',fontSize:'0.78rem'}}>Veri yok</div>; }
            })()}
          </div>
          <div style={cs}>
            <div style={{fontWeight:800,color:'#F5EBD7',marginBottom:'0.65rem',fontSize:'0.85rem'}}>🏛️ Partiler & Çeteler</div>
            {(() => {
              const parties = JSON.parse(localStorage.getItem('rep_parties')||'[]');
              const gangs = JSON.parse(localStorage.getItem('rep_gangs')||'[]');
              return (
                <div>
                  {[['🏛️','Parti Sayısı',parties.length,'#C89B3C'],['🔫','Çete/Aile',gangs.length,'#C2453D']].map(([ic,lbl,val,c])=>(
                    <div key={lbl} style={{display:'flex',justifyContent:'space-between',padding:'0.4rem 0',borderBottom:'1px solid rgba(255,255,255,0.04)',fontSize:'0.78rem'}}>
                      <span style={{color:'#A9A6A0'}}>{ic} {lbl}</span>
                  ...
                  {parties.slice(0,5).map(p=>(
                    <div key={p.id} style={{display:'flex',justifyContent:'space-between',padding:'0.3rem 0',borderBottom:'1px solid rgba(255,255,255,0.03)',fontSize:'0.72rem'}}>
                      <span style={{color:'#C89B3C'}}>🏛️ {p.name}</span>
                      <span style={{color:'#A9A6A0'}}>{p.memberCount||0} üye • 🪙{(p.treasury||0).toLocaleString('tr-TR')} hazine</span>
                    </div>
                  ))}
                </div>
              );
      ...
      {tab==='tools' && (
        <div>
          <div style={cs}>
            <div style={{fontWeight:800,color:'#F5EBD7',marginBottom:'0.75rem',fontSize:'0.85rem'}}>🛠️ Sistem Araçları</div>
            <div style={{display:'grid',gap:'0.5rem'}}>
              {[
                ['↻ Oyunu Yenile', '#C89B3C', ()=>window.location.reload()],
                ['👤 Profilime Git', '#C89B3C', ()=>onNavigate('profile')],
                ['🏠 Ana Sayfaya Git', '#4C9A6B', ()=>onNavigate('home')],
                ['🧹 Yerel Veriyi Temizle (DİKKAT!)', '#C2453D', ()=>{ if(window.confirm('TÜM yerel veriler silinecek! Emin misin?')){localStorage.clear();window.location.reload();} }],
              ].map(([lbl,clr,fn])=>(
          ...
          <div style={cs}>
            <div style={{fontWeight:800,color:'#F5EBD7',marginBottom:'0.5rem',fontSize:'0.85rem'}}>ℹ️ Sistem Bilgisi</div>
            {[
              ...
              <div key={k} style={{display:'flex',justifyContent:'space-between',padding:'0.4rem 0',borderBottom:'1px solid rgba(255,255,255,0.04)',fontSize:'0.78rem'}}>
                <span style={{color:'#A9A6A0'}}>{k}</span>
                <span style={{color:'#F5EBD7',fontWeight:700}}>{v}</span>
              </div>
            ))}
          </div>
        </div>
