...
function AuthScreen({ onLogin }) {
  ...
  const inputStyle = {
    ...
    border:'1px solid rgba(245,235,215,0.1)', background:'rgba(45,24,0,0.8)',
    color:'#F5EBD7', fontFamily:"'Inter',sans-serif", fontSize:'1rem',
    ...
    return (
      <div style={{position:'fixed',inset:0,display:'flex',flexDirection:'column',overflowY:'auto',minHeight:'100dvh'}}>
        ...
        <div style={{position:'relative',zIndex:2,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',minHeight:'100dvh',padding:'2rem 1rem'}}>
          <div style={{width:'100%',maxWidth:'420px',background:'rgba(45,24,0,0.92)',border:'1px solid rgba(200,155,60,0.25)',borderRadius:'10px',padding:'2rem 1.5rem',backdropFilter:'blur(24px)',boxShadow:'0 24px 80px rgba(0,0,0,0.7)',textAlign:'center'}}>
            <div style={{fontSize:'3rem',marginBottom:'0.75rem'}}>📧</div>
            <div style={{fontFamily:"'Cinzel',serif",fontSize:'1.15rem',fontWeight:800,color:'#C89B3C',marginBottom:'0.5rem',letterSpacing:'0.04em'}}>E-POSTANI DOĞRULA</div>
            <div style={{color:'#A9A6A0',fontSize:'0.85rem',lineHeight:1.6,marginBottom:'1.5rem'}}>
              <span style={{color:'#F5EBD7',fontWeight:600}}>{profile.email}</span> adresine bir doğrulama bağlantısı gönderdik.<br/>
              Maili açıp bağlantıya tıkladıktan sonra giriş yapabilirsin.
            </div>

            {resendMsg && (
              <div style={{background: resendMsg.startsWith('✅') ? 'rgba(76,154,107,0.1)' : 'rgba(194,69,61,0.1)', border: `1px solid ${resendMsg.startsWith('✅') ? 'rgba(76,154,107,0.3)' : 'rgba(194,69,61,0.3)'}`, borderRadius:'10px', padding:'0.6rem 0.8rem', marginBottom:'1rem', fontSize:'0.82rem', color: resendMsg.startsWith('✅') ? '#4C9A6B' : '#E08C87'}}>
                {resendMsg}
              </div>
            )}
            ...
            <button
              ...
              style={{width:'100%',padding:'0.9rem',borderRadius:'12px',border:'none',background:resendLoading?'rgba(200,155,60,0.25)':'rgba(200,155,60,0.12)',color:'#C89B3C',fontFamily:"'Inter',sans-serif",fontWeight:700,fontSize:'0.95rem',cursor:resendLoading?'not-allowed':'pointer',marginBottom:'0.75rem',border:'1px solid rgba(200,155,60,0.3)',display:'flex',alignItems:'center',justifyContent:'center',gap:'0.5rem',transition:'all 0.2s'}}>
              {resendLoading ? <><div style={{width:16,height:16,border:'2px solid rgba(200,155,60,0.3)',borderTopColor:'#C89B3C',borderRadius:'50%',animation:'spin 0.7s linear infinite'}} /> Gönderiliyor...</> : '📨 Tekrar Gönder'}
            </button>

            <button
              onClick={() => { _setupSocket(profile); onLogin(profile); }}
              style={{width:'100%',padding:'0.9rem',borderRadius:'12px',border:'1px solid rgba(245,235,215,0.1)',background:'rgba(245,235,215,0.04)',color:'#A9A6A0',fontFamily:"'Inter',sans-serif",fontWeight:600,fontSize:'0.88rem',cursor:'pointer',marginBottom:'0.75rem',transition:'all 0.2s'}}>
              Şimdilik Atla →
            </button>

            <button
              onClick={() => { setUnverifiedUser(null); setResendMsg(''); }}
              style={{background:'none',border:'none',color:'#A9A6A0',fontSize:'0.78rem',cursor:'pointer',fontFamily:"'Inter',sans-serif",textDecoration:'underline',padding:'4px 8px'}}>
    ...
    <div style={{position:'fixed',inset:0,display:'flex',flexDirection:'column',overflowY:'auto',minHeight:'100dvh'}}>
      ...
      <div style={{position:'relative',zIndex:2,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',minHeight:'100dvh',padding:'env(safe-area-inset-top,1rem) 0 env(safe-area-inset-bottom,1rem)'}}>
        ...
        <div style={{textAlign:'center',marginBottom:'1.5rem',width:'100%',padding:'0 1.5rem'}}>
          ...
          <div style={{display:'flex',justifyContent:'center',marginBottom:'0.5rem'}}>
            <img src="saltanat-icon.png" alt="Saltanat Online" style={{width:'clamp(90px,22vw,140px)',height:'clamp(90px,22vw,140px)',objectFit:'contain',borderRadius:'20%',boxShadow:'0 0 40px rgba(200,155,60,0.4)',border:'2px solid rgba(200,155,60,0.25)'}} />
          </div>
          <div style={{fontFamily:"'Cinzel',serif",fontSize:'clamp(1.4rem,5vw,2.2rem)',fontWeight:900,letterSpacing:'0.15em',color:'#F5EBD7',textShadow:'0 2px 24px rgba(0,0,0,0.8)',textTransform:'uppercase',marginBottom:'0.4rem'}}>
            SALTANAT ONLİNE
          </div>
          {/* Animated loading bar */}
          <div style={{width:'100%',maxWidth:'320px',margin:'0 auto',height:'3px',background:'rgba(245,235,215,0.12)',borderRadius:'2px',overflow:'hidden'}}>
            <div style={{height:'100%',width:loading?`${barProgress}%`:'0%',background:'linear-gradient(90deg,#C89B3C,#E8C06A,#C89B3C)',borderRadius:'2px',transition:loading?'width 1.8s cubic-bezier(0.4,0,0.2,1)':'none',boxShadow:'0 0 8px rgba(200,155,60,0.8)'}} />
          </div>
          <div style={{color:'#A9A6A0',fontSize:'0.65rem',letterSpacing:'0.15em',textTransform:'uppercase',fontFamily:"'Inter',sans-serif",marginTop:'0.4rem'}}>
        ...
        <div style={{width:'100%',maxWidth:'480px',padding:'0 1rem'}}>
          ...
          <div style={{background:'rgba(30,15,0,0.93)',border:'1px solid rgba(200,155,60,0.18)',borderRadius:'22px',padding:'1.85rem 1.6rem',backdropFilter:'blur(30px)',WebkitBackdropFilter:'blur(30px)',boxShadow:'0 28px 80px rgba(0,0,0,0.75), 0 0 32px rgba(200,155,60,0.08), inset 0 1px 0 rgba(255,255,255,0.05)'}}>
              ...
              {[['login','→ '+T('loginTab')],['register',T('registerTab')]].map(([v,l]) => (
                <button key={v} type="button" onClick={() => {setTab(v);setErr('');}}
                  style={{
                    ...
                    border: tab===v ? '1.5px solid #C89B3C' : '1px solid rgba(245,235,215,0.1)',
                    background: tab===v ? 'rgba(200,155,60,0.12)' : 'rgba(245,235,215,0.04)',
                    color: tab===v ? '#C89B3C' : 'rgba(245,235,215,0.4)',
                    boxShadow: tab===v ? '0 2px 14px rgba(200,155,60,0.2)' : 'none',
                  }}>
                  {l}
                </button>
              ))}
            ...
            {/* Error */}
            {err && <div style={{background:'rgba(194,69,61,0.1)',border:'1px solid rgba(194,69,61,0.25)',borderRadius:'12px',padding:'0.7rem 1rem',color:'#E08C87',fontSize:'0.85rem',marginBottom:'1rem',display:'flex',alignItems:'center',gap:'0.5rem'}}>⚠️ {err}</div>}
            ...
            <div style={{marginBottom:'1.25rem',position:'relative'}}>
              ...
              <button type="button" onClick={()=>setShowPw(p=>!p)} style={{position:'absolute',right:'1rem',top:'50%',transform:'translateY(-50%)',background:'none',border:'none',color:'#A9A6A0',cursor:'pointer',fontSize:'1rem',padding:'4px'}}>{showPw?'🙈':'👁️'}</button>
            </div>
            ...
            {tab==='register' && <>
              ...
              <div style={{marginBottom:'1rem'}}>
                <select value={f.city} onChange={e=>u('city',e.target.value)}
                  style={{...inputStyle,color:f.city?'#F5EBD7':'rgba(255,255,255,0.4)'}}>
                  {CITIES.map(c=><option key={c} value={c} style={{background:'#2D1800'}}>{c}</option>)}
                </select>
              </div>
              <div style={{marginBottom:'1rem'}}>
            ...
            <button type="submit" disabled={loading}
              style={{width:'100%',padding:'1.05rem',borderRadius:'12px',border:'none',background:loading?'rgba(200,155,60,0.25)':'linear-gradient(135deg,#C89B3C,#C89B3C,#A07828)',color:loading?'rgba(255,255,255,0.5)':'#0A0800',fontFamily:"'Cinzel',serif",fontWeight:900,fontSize:'1rem',letterSpacing:'0.14em',cursor:loading?'not-allowed':'pointer',transition:'all 0.2s ease',textTransform:'uppercase',boxShadow:loading?'none':'0 6px 28px rgba(200,155,60,0.45), inset 0 1px 0 rgba(255,255,255,0.2)',display:'flex',alignItems:'center',justifyContent:'center',gap:'0.5rem'}}>
              {loading ? <>
            ...
            {tab==='login' && (
              <div style={{textAlign:'center',marginTop:'1rem'}}>
                <button type="button"
                  ...
                  style={{background:'none',border:'none',color:'#A9A6A0',fontSize:'0.8rem',cursor:'pointer',fontFamily:"'Inter',sans-serif",textDecoration:'underline',textDecorationColor:'rgba(255,255,255,0.15)',letterSpacing:'0.02em',padding:'4px 8px'}}
        ...
        <div style={{marginTop:'0.75rem',color:'rgba(245,235,215,0.2)',fontSize:'0.68rem',textAlign:'center',position:'relative',zIndex:1,letterSpacing:'0.08em'}}>
          🔒 Saltanat Online • Güvenli Giriş
        </div>
        {/* Günün İpucu */}
        <div style={{marginTop:'0.85rem',padding:'0.6rem 0.85rem',background:'rgba(200,155,60,0.06)',border:'1px solid rgba(200,155,60,0.15)',borderRadius:'10px',position:'relative',zIndex:1,animation:'notifIn 0.5s ease',maxWidth:'360px',alignSelf:'center',width:'calc(100% - 2rem)'}}>
          <div style={{fontSize:'0.62rem',color:'rgba(200,155,60,0.6)',fontWeight:700,letterSpacing:'0.08em',marginBottom:'0.25rem',fontFamily:"'Cinzel',serif"}}>💡 TARİHİ BİLGİ</div>
          <div style={{fontSize:'0.75rem',color:'rgba(245,235,215,0.65)',lineHeight:1.5}}>{OTTOMAN_TIPS[tipIdx]}</div>
          <div style={{display:'flex',justifyContent:'center',gap:'4px',marginTop:'0.4rem'}}>
            {OTTOMAN_TIPS.map((_,i) => (
              <div key={i} onClick={()=>setTipIdx(i)} style={{width:i===tipIdx?14:5,height:4,borderRadius:'2px',background:i===tipIdx?'rgba(200,155,60,0.7)':'rgba(200,155,60,0.2)',cursor:'pointer',transition:'all 0.3s'}} />
            ))}
          </div>
        </div>
      </div>
      ...
      {showForgot && (
        <div style={{position:'fixed',inset:0,zIndex:200,display:'flex',alignItems:'center',justifyContent:'center',padding:'1.25rem',background:'rgba(0,0,0,0.7)',backdropFilter:'blur(8px)'}}>
          <div style={{background:'#2D1800',border:'1px solid rgba(245,235,215,0.08)',borderRadius:'14px',padding:'1.75rem 1.5rem',width:'100%',maxWidth:'380px',boxShadow:'0 24px 64px rgba(0,0,0,0.6)'}}>
            <div style={{textAlign:'center',marginBottom:'1.25rem'}}>
              <div style={{fontSize:'2rem',marginBottom:'0.35rem'}}>🔑</div>
              <div style={{fontFamily:"'Cinzel',serif",fontWeight:800,color:'#F5EBD7',fontSize:'1.2rem',marginBottom:'0.25rem'}}>Şifremi Unuttum</div>
              <div style={{fontSize:'0.78rem',color:'#A9A6A0'}}>E-posta adresinize sıfırlama bağlantısı göndereceğiz</div>
            </div>
            {forgotMsg && (
              <div style={{background:forgotMsg.startsWith('✅')?'rgba(76,154,107,0.1)':'rgba(194,69,61,0.1)',border:`1px solid ${forgotMsg.startsWith('✅')?'rgba(76,154,107,0.3)':'rgba(194,69,61,0.3)'}`,borderRadius:'10px',padding:'0.6rem 0.85rem',marginBottom:'0.85rem',fontSize:'0.82rem',color:forgotMsg.startsWith('✅')?'#4C9A6B':'#E08C87',textAlign:'center'}}>
                {forgotMsg}
              </div>
            )}
            <input
              ...
              style={{width:'100%',background:'rgba(245,235,215,0.03)',border:'1px solid rgba(245,235,215,0.1)',borderRadius:'10px',padding:'0.75rem 1rem',color:'#F5EBD7',fontFamily:"'Inter',sans-serif",fontSize:'16px',outline:'none',boxSizing:'border-box',marginBottom:'0.75rem'}}
            ...
            }} style={{width:'100%',padding:'0.85rem',borderRadius:'10px',border:'none',background:forgotLoading?'rgba(200,155,60,0.25)':'#C89B3C',color:'#0F0800',fontFamily:"'Inter',sans-serif",fontWeight:700,fontSize:'0.95rem',cursor:forgotLoading?'not-allowed':'pointer',marginBottom:'0.5rem',transition:'all 0.2s'}}>
              {forgotLoading ? 'Gönderiliyor...' : '📨 Bağlantı Gönder'}
            </button>
            <button onClick={()=>setShowForgot(false)} style={{width:'100%',padding:'0.65rem',borderRadius:'10px',border:'1px solid rgba(245,235,215,0.08)',background:'transparent',color:'#A9A6A0',fontFamily:"'Inter',sans-serif",fontWeight:600,fontSize:'0.9rem',cursor:'pointer'}}>
              İptal
            </button>
          </div>
        </div>
      ...
      {showReset && (
        <div style={{position:'fixed',inset:0,zIndex:200,display:'flex',alignItems:'center',justifyContent:'center',padding:'1.25rem',background:'rgba(0,0,0,0.7)',backdropFilter:'blur(8px)'}}>
          <div style={{background:'#2D1800',border:'1px solid rgba(245,235,215,0.08)',borderRadius:'14px',padding:'1.75rem 1.5rem',width:'100%',maxWidth:'380px',boxShadow:'0 24px 64px rgba(0,0,0,0.6)'}}>
            <div style={{textAlign:'center',marginBottom:'1.25rem'}}>
              <div style={{fontSize:'2rem',marginBottom:'0.35rem'}}>🔒</div>
              <div style={{fontFamily:"'Cinzel',serif",fontWeight:800,color:'#F5EBD7',fontSize:'1.2rem',marginBottom:'0.25rem'}}>Yeni Şifre Belirle</div>
              <div style={{fontSize:'0.78rem',color:'#A9A6A0'}}>Güçlü bir şifre seçin (min 8 karakter)</div>
            </div>
            {resetMsg && (
              <div style={{background:resetMsg.startsWith('✅')?'rgba(76,154,107,0.1)':'rgba(194,69,61,0.1)',border:`1px solid ${resetMsg.startsWith('✅')?'rgba(76,154,107,0.3)':'rgba(194,69,61,0.3)'}`,borderRadius:'10px',padding:'0.6rem 0.85rem',marginBottom:'0.85rem',fontSize:'0.82rem',color:resetMsg.startsWith('✅')?'#4C9A6B':'#E08C87',textAlign:'center'}}>
                {resetMsg}
              </div>
            )}
            <input
              ...
              style={{width:'100%',background:'rgba(245,235,215,0.03)',border:'1px solid rgba(245,235,215,0.1)',borderRadius:'10px',padding:'0.75rem 1rem',color:'#F5EBD7',fontFamily:"'Inter',sans-serif",fontSize:'16px',outline:'none',boxSizing:'border-box',marginBottom:'0.5rem'}}
            />
            <input
            ...
            }} style={{width:'100%',padding:'0.85rem',borderRadius:'10px',border:'none',background:resetLoading?'rgba(200,155,60,0.3)':'#C89B3C',color:'#0F0800',fontFamily:"'Inter',sans-serif",fontWeight:700,fontSize:'0.95rem',cursor:resetLoading?'not-allowed':'pointer',marginBottom:'0.5rem',transition:'all 0.2s'}}>
              {resetLoading ? 'Güncelleniyor...' : '🔒 Şifreyi Kaydet'}
            </button>
            <button onClick={()=>setShowReset(false)} style={{width:'100%',padding:'0.65rem',borderRadius:'10px',border:'1px solid rgba(245,235,215,0.08)',background:'transparent',color:'#A9A6A0',fontFamily:"'Inter',sans-serif",fontWeight:600,fontSize:'0.9rem',cursor:'pointer'}}>
              İptal
            </button>
          </div>
        </div>
